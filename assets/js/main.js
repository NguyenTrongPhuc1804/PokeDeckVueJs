...
Vue.createApp({
  ...
  watch: {
    "gameData.badSelect"(newValue) {
      if (newValue <= 0) {
        this.gameResult.lost = true;
      }
    },
    "gameData.selectDeck"(deck) {
      console.log(deck);
      this.gameData.selectDeck = deck;
      this.handleResetGame(this.gameData.selectDeck);
    },
  },
  computed: {
    covered() {
      let coveredCard = this.pokemons.filter(
        (item) => !this.uncoveredCard.includes(item)
      );
    ...
    uncoveredCard() {
      let uncovered = [...this.selected, ...this.pairedCard];
      return uncovered;
    },
  },
  methods: {
    ...
    handleResetGame(deck) {
      this.pokemons = this.createPokemonByDeck(this.gameData.selectDeck).sort(
        () => Math.random() - 0.5
      );
      this.selected = [];
      this.pairedCard = [];
    ...
    createPokemonByDeck(deck) {
      const randomPoke = Math.floor(Math.random() * 88);
      let pokemonCardList = arrPokemons.splice(randomPoke, deck);
      const pokemonCardListDouble = [];
      pokemonCardList.map((poke) => {
        const pokemonCard1 = { ...poke };
        const pokemonCard2 = { ...poke };
        pokemonCardListDouble.push(pokemonCard1);
        pokemonCardListDouble.push(pokemonCard2);
      });
      return pokemonCardListDouble;
    },
  },
}).mount("#app");
